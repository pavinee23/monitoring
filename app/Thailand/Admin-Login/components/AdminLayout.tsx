...
const menuItems: MenuItem[] = [
  {
    label: 'Dashboard',
    labelTh: 'แดชบอร์ด',
    href: '/Thailand/Admin-Login/dashboard',
    icon: (
    ...
    { label: 'Purchase Order', labelTh: 'ใบสั่งซื้อ', href: '/Thailand/Admin-Login/purchase-order', icon: (
      ...
      { label: 'Customer Payments', labelTh: 'รายการชำระเงินลูกค้า', href: '/Thailand/Admin-Login/customer-pay', icon: (
      ...
      { label: 'Pending Bills', labelTh: 'รายการบิลรออนุมัติ', href: '/Thailand/Admin-Login/receipt/pending', icon: (
      ...
      { label: 'Contract', labelTh: 'Contract', href: '/Thailand/Admin-Login/contract/list', icon: (
      ...
      { label: 'Products', labelTh: 'สินค้า', href: '/Thailand/Admin-Login/product-add', icon: (
      ...
      { label: 'Power Calculator List', labelTh: 'Power Calculator List', href: '/Thailand/Admin-Login/power-calculator/list', icon: (
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 7h18"/><path d="M3 12h18"/><path d="M3 17h18"/></svg>
        ) }
      ];
    ...
    <div className={styles.adminLayout}>
      ...
      <header className={styles.topHeader}>
        <div className={styles.headerLogo}>
          <img src="/k-energy-save-logo.jpg" alt="K Energy Save Co., Ltd." />
          <div>
            <div className={styles.headerTitle}>K Energy Save Co., Ltd.</div>
        ...
        <div className={styles.headerNav}>
          <div className={`${styles.langPrintGroup} no-print`}>
            <div className={styles.langPill}>
              <button onClick={() => setLang('th')} className={`${styles.btnLocale} ${lang === 'th' ? styles.localeActive : ''}`}>ไทย</button>
              <button onClick={() => setLang('en')} className={`${styles.btnLocale} ${lang === 'en' ? styles.localeActive : ''}`}>EN</button>
            </div>
          </div>
        </div>
        ...
        <aside className={`${styles.sidebar} no-print`}>
          <div className={styles.sidebarCard}>
            ...
            <nav className={styles.sidebarMenu}>
              {menuItems
                .filter(item => {
                  try {
                    // If allowedPages not yet loaded, show items for admin users or show all as fallback
                    if (allowedPages === null) return true
                    // if user is admin types, allow all
                    ...
                    // otherwise only show when allowedPages contains the exact route
          ...
          <div className={styles.sidebarCard} style={{ marginTop: 12 }}>
            ...
            <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <Link href="/Thailand/Admin-Login/products/list" className={styles.sidebarMenuItem}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M3 7h18M3 12h18M3 17h18" />
                  </svg>
                  <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1 }}>
                    <span style={{ fontWeight: 600 }}>{L('Products', 'สินค้า')}</span>
                    <span style={{ fontSize: 12, color: '#777' }}>{L('product view', 'ดูสินค้า')}</span>
                  </div>
                </span>
              </Link>
            </div>
      ...
      <footer className={styles.footer}>
        © 2026 K Energy Save Co., Ltd. All rights reserved.
      </footer>
